...
from scipy.sparse import diags, kron, eye, dia_matrix
import numpy as np
...
def steepest_descent(A: dia_matrix,
                     b: np.ndarray,
                     x_0: np.ndarray,
                     epsilon: float,
                     max_iter: int,
...
def conjugate_gradient(A: dia_matrix,
    ...
    while sqrt(norm_squared) > epsilon and k < max_iter:
        ...
        alpha = norm_squared/(res.dot(A_dot_p))
...
I = eye(n)
K_d1 = diags([subdiag, diag, subdiag], offsets=[-1, 0, 1])
...
print("\nParameters:\nN:", N, "\nEpsilon:",
      epsilon, "Max iterations:", max_iter)

print("\nSteepest descent")
x, k, error_seq = timeit(
    steepest_descent, [A, b, x_0, epsilon, max_iter, x_star])
e_0 = error_seq[0]
error_seq = np.array(error_seq)

print("iterations:", k, "\nerror norm:", np.linalg.norm(
    x - x_star), "\nerror energy norm:", error_seq[-1])
plt.semilogy(range(len(error_seq)), error_seq/e_0, 'b', label="SD")
